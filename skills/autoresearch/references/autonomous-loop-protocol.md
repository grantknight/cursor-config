...
## Phase 5a: Regression Gate (Absolute)

After Verify, re-run the regression suite (`regression_cmd`). If `passed < N_pre` (the baseline recorded in `autoresearch/.regression-baseline`), short-circuit to `discard` — do not evaluate the metric. A tempting win that drops pre-existing passing tests is never a win.

```
IF regression_passed < N_pre:
